...
def search(agent, env, config):
    ...
    while episode < num_episode:  # counting based on episode
        ...
        if observation is None:
            ...
            env.reset()
            observation = deepcopy(env.layer_embedding[0].copy())
        ...
        if done:  # end of episode
            if debug:
                print('#{}: episode_reward:{:.4f} acc: {:.4f}, w_ratio: {:.4f}'.format(episode, episode_reward,
                                                                                         info['accuracy'],
                                                                                         info['w_ratio']))
            text_writer.write(
                '#{}: episode_reward:{:.4f} acc: {:.4f}, w_ratio: {:.4f}\n'.format(episode, episode_reward,
                                                                                     info['accuracy'],
                                                                                     info['w_ratio']))
            final_reward = T[-1][0]
